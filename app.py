...
# B. DYNAMIC ORDER BOOK
with col_depth:
    st.markdown("<div class='ob-header'>📊 Live Order Book</div>", unsafe_allow_html=True)
    if last_price > 0:
        step = last_price * 0.0003
        asks_df = pd.DataFrame({
            "Price ($)": [round(last_price + (i * step), 2) for i in range(5, 0, -1)],
            "Size": [0.35, 1.12, 0.08, 2.45, 0.91]
        })
        bids_df = pd.DataFrame({
            "Price ($)": [round(last_price - (i * step), 2) for i in range(1, 6)],
            "Size": [1.45, 0.62, 3.12, 0.85, 1.10]
        })
        
        st.caption("🔴 Sell Orders (Asks)")
        st.dataframe(asks_df.style.format({"Price ($)": "${:,.2f}", "Size": "{:.2f}"}), use_container_width=True, height=170, hide_index=True)
        ...
        st.dataframe(bids_df.style.format({"Price ($)": "${:,.2f}", "Size": "{:.2f}"}), use_container_width=True, height=170, hide_index=True)
    else:
...
with tab_pos:
    if last_price > 0:
        # Dynamic calculation based on REAL market price
